-- Bazaar Marketplace Database Schema v2 (No DROP version)
-- PostgreSQL schema with UUIDs, Enums, and Audit Triggers
-- Use this for updates to existing databases
...
DROP TRIGGER IF EXISTS set_timestamp_sellers ON sellers;
...
CREATE INDEX IF NOT EXISTS idx_products_published_not_sold ON products(is_published, is_sold);
CREATE INDEX IF NOT EXISTS idx_orders_seller_status ON orders(seller_id, status);
CREATE INDEX IF NOT EXISTS idx_offers_request_status ON offers(buyer_request_id, offer_status);
CREATE INDEX IF NOT EXISTS idx_users_geo ON users(country, city);
...
-- Sample Data (only insert if categories table is empty)
INSERT INTO categories (name, icon_name, bg_color, text_color)
SELECT 'Apparel', 'Shirt', 'bg-red-100', 'text-red-600'
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Apparel');

INSERT INTO categories (name, icon_name, bg_color, text_color)
SELECT 'Tech', 'Smartphone', 'bg-blue-100', 'text-blue-600'
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Tech');

INSERT INTO categories (name, icon_name, bg_color, text_color)
SELECT 'Home', 'Home', 'bg-green-100', 'text-green-600'
