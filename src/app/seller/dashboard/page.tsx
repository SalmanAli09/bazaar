...
export default function SellerHub() {
  ...
  // Load seller products from mock data
  useEffect(() => {
    const loadSellerData = async () => {
      try {
        const sellerData = await import('../../../../data/seller_flow.json');
        const products = sellerData.default.products || [];
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-10 shadow-sm">
        <div className="p-6">
          <div className="flex items-center gap-3 mb-10">
            <div className="size-10 rounded-2xl bg-[#11d421] flex items-center justify-center text-slate-900 shadow-lg shadow-[#11d421]/20">
              <Store size={22} strokeWidth={2.5} />
            </div>
            <div>
              <h1 className="text-slate-900 text-base font-black leading-tight">Bazaar</h1>
          ...
          <nav className="flex flex-col gap-2">
            {navItems.map((item) => (
              ...
              >
                ...
                {item.badge && (
                  <span className="ml-auto bg-[#11d421] text-slate-900 text-[10px] px-2 py-0.5 rounded-full">
                    {item.badge}
                  </span>
                )}
              </button>
        ...
        <div className="mt-auto p-6 border-t border-slate-100">
          <button className="w-full bg-[#11d421] text-slate-900 font-black py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 text-sm shadow-xl shadow-[#11d421]/20 hover:scale-[1.02] active:scale-95 transition-all">
            <PlusCircle size={18} />
            New Listing
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-black/[0.02]">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="p-2 bg-[#11d421]/10 rounded-xl text-[#11d421]"><Wallet size={20}/></div>
            </div>
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-slate-900">$1,240.50</p>
              <span className="text-[#11d421] text-xs font-black flex items-center gap-0.5">
                <TrendingUp size={14} /> +12%
              </span>
            </div>
            <button className="mt-8 w-full py-3 bg-slate-900 text-white font-black rounded-2xl text-xs hover:bg-slate-800 transition-all uppercase tracking-widest">
              Withdraw Funds
            </button>
          </div>
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-black/[0.02]">
            ...
            <div className="mt-8">
              <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div className="bg-[#11d421] h-full w-[70%] rounded-full shadow-[0_0_10px_rgba(17,212,33,0.4)]"></div>
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {sellerProducts.map((product) => (
                    <tr key={product.product_id} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="size-14 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-100 group-hover:scale-105 transition-transform">
                             <img className="w-full h-full object-cover" src={`https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=100&q=80`} alt={product.product_name} />
                          </div>
                          <div>
                            <p className="text-sm font-black text-slate-900">{product.product_name}</p>
                      ...
                      <td className="px-8 py-5">
                        <div className="flex flex-col gap-1">
                          <span className="text-sm font-black text-slate-700">${product.product_selling_price.toFixed(2)}</span>
                          {product.product_actual_price > product.product_selling_price && (
                            <span className="text-xs text-slate-400 line-through">${product.product_actual_price.toFixed(2)}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <span className={`px-3 py-1 text-[10px] font-black rounded-lg uppercase tracking-widest ${
                          ...
                          {product.is_sold ? 'Sold' : product.is_draft ? 'Draft' : 'Active'}
                        </span>
                        {product.is_urgent && (
                          <span className="ml-2 px-2 py-1 bg-red-100 text-red-600 text-[8px] font-black rounded-lg uppercase tracking-widest">
                            Urgent
                          </span>
                        )}
                        {product.is_featured && (
                          <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-600 text-[8px] font-black rounded-lg uppercase tracking-widest">
                            Featured
                          </span>
                        )}
                      </td>
                      <td className="px-8 py-5 text-right">
                        <div className="flex justify-end gap-2">
                          <button className="p-2.5 hover:bg-[#11d421]/10 rounded-xl text-slate-400 hover:text-[#11d421] transition-all">
                            <Pencil size={18} />
                          </button>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-black/[0.02] flex flex-col">
            ...
            <div className="p-4 space-y-2 overflow-y-auto max-h-[420px]">
              {[
                { title: "New Order!", desc: "Sold '90s Tee' to @alex_m", icon: ShoppingBag, color: "bg-[#11d421]/10 text-[#11d421]", time: "2m ago" },
            ...
            <div className="mt-auto p-6 border-t border-slate-50 text-center">
              <button className="flex items-center justify-center gap-2 w-full text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] hover:text-[#11d421] transition-colors">
