  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
      <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
        <Images className="w-5 h-5 text-[var(--primary-dark)]" />
        Upload Photos
      </h2>
      ...
      <div className="grid grid-cols-3 gap-3">
        {formData.product_pictures.map((picture, index) => (
          <div key={index} className="aspect-square rounded-xl overflow-hidden relative group">
            ...
            {index === 0 && (
              <div className="absolute top-2 left-2 bg-[var(--primary-dark)] text-white text-xs px-2 py-1 rounded">
                Cover
              </div>
            )}
          </div>
        ...
        {formData.product_pictures.length < 5 && (
          <label className="aspect-square rounded-xl border-2 border-dashed border-primary bg-[var(--primary-dark)]/5 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-[var(--primary-dark)]/10 transition-colors group">
            <Camera className="w-6 h-6 text-[var(--primary-dark)] group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-bold text-[var(--primary-dark)] uppercase tracking-wider">
      ...
      <label className="w-full mt-6 py-2.5 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold hover:border-primary hover:text-[var(--primary-dark)] transition-all flex items-center justify-center gap-2 cursor-pointer">
